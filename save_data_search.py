...
def save_weather_now(lists, username):

    sql = MysqlPython(database=username)
...
# 一周天气显示
def save_weather_week(lists, username):

    sql = MysqlPython(database=username)
    # print(lists)

    sql_database = 'use {}'.format(username)
    sql.execute_database(sql_database)
...
# 天气指数显示
def save_index_week(lists, username):
